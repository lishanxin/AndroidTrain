...
public class CameraActivity extends Activity implements OnClickListener {
	...
	private void layoutInit() {
		...
		// 获取SurfaceView以便显示Camera传来的图像
		surfaceView = (SurfaceView) findViewById(R.id.camera_surfaceview);
		// 根据SurfaceView的容器来重新计算SurfaceView的显示尺寸
	...
	private final class MyPictureCallback implements PictureCallback {
		...
		public void onPictureTaken(byte[] data, Camera camera) {
			...
			if (cropCamera) {
				...
				int extendH = 0;
				// 增加部分截取边距
				int x = (int) (bOld.getWidth() * (1 - 0.8) / 2 - extendW);
				...
				int cropWidth = (int) (bOld.getWidth() * 0.8 + extendW * 2);
				...
				bNew[takeIndex] = Bitmap.createBitmap(bOld, x, y, cropWidth, cropHeight);
				if (canRotaion) {
					iv.setImageBitmap(bNew[takeIndex]);
				}
